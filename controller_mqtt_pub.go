...
func main() {
	...
	message:="SensorRead"+os.Args[1]
	result, err := contract.EvaluateTransaction("querySensorRead", message)
	if err != nil {
		fmt.Printf("Failed to evaluate transaction: %s\n", err)
		os.Exit(1)
	}
	fmt.Println(string(result))
	sendingMessage:=string(result)
	...
	payload := flag.String("message", sendingMessage, "The message text to publish (default empty)")
